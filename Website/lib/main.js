...
$('#submit-button').click(function (){
    // var ttext = document.getElementById('transcript').value;
    var httpRequest = new XMLHttpRequest();
    var ttext = $('#transcript').val();
    var url = 'https://<APIGW_ID>.execute-api.us-east-1.amazonaws.com/prod/ttext?message=' + ttext;
    if (ttext === null || ttext === '') {
        alert("Please speak something on the mic.");
        return false;
    }
    else {
    ...
    function alertContents() {
        if (httpRequest.readyState === XMLHttpRequest.DONE) {
          if (httpRequest.status === 200) {
            ...
            console.error('Failed to upload transcribe text: ' + ttext);
          }
        }
      }
 });
